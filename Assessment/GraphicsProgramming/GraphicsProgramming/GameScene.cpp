#include "GameScene.h"
// TODO thread (103)
GameScene::GameScene(Input *in)
{
	cout << "Game Scene constructor called!\n";
	Init(in);
}

void GameScene::Init(Input *in)
{
	cout << "Game Scene constructor start\n";
	// game state
	//game_state = LOAD;
	...
	trilinear = false;
	right = true;
	cout << "Game Scene constructor end\n";
}
...
void GameScene::setRenderMode(bool blend, bool wireframe) {
...
// TODO thread (143)
void GameScene::renderStencilBuffer() {
...
void GameScene::renderShadowing() {
...
void GameScene::renderStencilShadowing() {
	// Stencil buffer settings
	glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);	// Turn off writing to the frame buffer
	glEnable(GL_STENCIL_TEST);								// Enable the stencil test
	glStencilFunc(GL_ALWAYS, 1, 1);							// Set the stencil function to always pass
...
// TODO thread (16 - butterfly takes up the most time (1))
void GameScene::buildShapes() {
	skybox.buildFromArray(
		GL_TRIANGLES,
		...
		skybox_texcoords,
		skybox_tex,
		GET_VARIABLE_NAME(skybox));
	// Sphere's for material altering
	// 5x5 spheres
	sphere_1.buildSphere(
		GL_TRIANGLES, 0.5f, 5.0f, 5.0f,	// radius, latitude, longitude
		...
		blue_tex,
		GET_VARIABLE_NAME(sphere_1));
	// 10x10 sphere
	sphere_2.buildSphere(
		GL_TRIANGLES, 0.5f, 10.0f, 10.0f,	// radius, latitude, longitude
		Vector3(0.0f, 2.0f, -2.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_2));
	// 15x15 sphere
	sphere_3.buildSphere(
		GL_TRIANGLES, 0.5f, 10.0f, 15.0f,	// radius, latitude, longitude
		Vector3(0.0f, 2.0f, -4.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_3));
	// 15x10 sphere
	sphere_4.buildSphere(
		GL_TRIANGLES, 0.5f, 15.0f, 10.0f,	// radius, latitude, longitude
		Vector3(0.0f, 2.0f, -6.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_4));
	// 30x30 sphere
	sphere_5.buildSphere(
		GL_TRIANGLES, 0.5f, 30.0f, 30.0f,	// radius, latitude, longitude
		Vector3(0.0f, 0.0f, 0.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_5));
	// 45x30 sphere
	sphere_6.buildSphere(GL_TRIANGLES, 0.5f, 45.0f, 30.0f,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_6));
	// 80x80 sphere
	sphere_7.buildSphere(
		GL_TRIANGLES, 0.5f, 80.0f, 80.0f,	// radius, latitude, longitude
		Vector3(0.0f, 0.0f, -4.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_7));
	// 160x160 sphere
	sphere_8.buildSphere(
		GL_TRIANGLES, 0.5f, 160.0f, 160.0f,	// radius, latitude, longitude
		Vector3(0.0f, 0.0f, -6.0f),							// translate x, translate y, translate z,
		Vector3(1.0f, 1.0f, 1.0f),							// scale x, scale y, scale z,
		Vector4(0.0, 0.0, 1.0, 0.0),						// rotation angle, rotation x, rotation y, rotation z
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),					// red, green, blue, alpha colour
		blue_tex,
		GET_VARIABLE_NAME(sphere_8));
	
	disc_1.buildDisc(
		GL_TRIANGLE_FAN, 32.0f, 2.0f,
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 0.0, 0.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(disc_1));

	disc_2.buildDisc(
		GL_TRIANGLE_FAN, 8.0f, 2.0f,		// edges, radius
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 0.0, 1.0, 0.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(disc_2));

	disc_3.buildDisc(
		GL_TRIANGLE_FAN, 6.0f, 2.0f,		// edges, radius
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 0.0, 0.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(disc_3));
	...
	circle.buildCircle(
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(circle));

	floor.buildFromArray(
		GL_TRIANGLES,
		...
		quad_t_texcoords,
		NULL,
		GET_VARIABLE_NAME(floor));

	main_floor.buildFromArray(
		GL_TRIANGLES,
		...
		quad_t_texcoords,
		grass_tex,
		GET_VARIABLE_NAME(main_floor));

	cone_1.buildCone(
		GL_TRIANGLES, 2.0f, 10.0f, 10.0f,
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 0.0, 1.0, 0.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(cone_1));
	...
	cone_2.buildCone(
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(cone_2));

	cylinder_1.buildCylinder(
		GL_TRIANGLES, 2.3f, 20.0f, 10.f,
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(-1.0f, -1.0f, 1.0f),
		Vector4(1.0f, 0.0f, 1.0f, 0.0f),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		barrel_tex,
		GET_VARIABLE_NAME(cylinder_1));

	cylinder_2.buildCylinder(
		GL_TRIANGLES, 2.3f, 20.0f, 10.f,
		Vector3(0.0f, 0.0f, 0.0f),
		Vector3(-1.0f, -1.0f, 1.0f),
		Vector4(1.0f, 0.0f, 1.0f, 0.0f),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		barrel_tex,
		GET_VARIABLE_NAME(cylinder_2));

	//Toruses
	torus_1.buildTorus(
		GL_TRIANGLES, 2.0, 4.0, 20.0, 10.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges
		Vector3(12.0f, 0.0f, 0.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_1));

	torus_2.buildTorus(
		GL_TRIANGLES, 2.0, 4.0, 32.0, 32.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges
		Vector3(12.0f, 0.0f, 8.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(45.0, 1.0, 0.0, 0.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_2));

	torus_3.buildTorus(
		GL_TRIANGLES, 2.0, 4.0, 64.0, 64.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges
		Vector3(12.0f, 4.0f, 4.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(-45.0, 1.0, 0.0, 0.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_3));

	torus_pres.buildTorus(
		GL_TRIANGLES, 2.0, 4.0, 128.0, 128.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges
		Vector3(15.0f, 6.0f, 20.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_pres));

	torus_three.buildTorus(
		GL_TRIANGLES, 1.0, 2.0, 3.0, 32.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges 
		Vector3(-14.0f, 7.0f, 20.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_three));

	torus_three_1.buildTorus(
		GL_TRIANGLES, 1.0, 2.0, 32.0, 3.0,	// r - radius of the tube, R - distance from the center of the tube to the center of the torus, torus edges, tube edges
		Vector3(-10.0f, 4.0f, 18.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		doughnut_tex,
		GET_VARIABLE_NAME(torus_three_1));

	disc_pres.buildDisc(
		GL_TRIANGLE_FAN, 128.0f, 2.0f,		// edges, radius
		Vector3(2.0f, 3.0f, 10.0f),
		Vector3(2.0f, 2.0f, 2.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(disc_pres));

	cylinder_pres.buildCylinder(
		GL_TRIANGLES, 2.3f, 20.0f, 10.f,
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		barrel_tex,
		GET_VARIABLE_NAME(cylinder_pres));

	sphere_pres.buildSphere(
		GL_TRIANGLES, 2.0f, 64.0f, 64.0f,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),						// red, green, blue, alpha colour
		earth_clouds_tex,
		GET_VARIABLE_NAME(sphere_pres));

	cone_pres.buildCone(
		GL_TRIANGLES, 2.0f, 64.0f, 5.0f,
		Vector3(-10.0f, 9.0f, 20.0f),
		Vector3(1.0f, 1.0f, 1.0f),
		Vector4(0.0, 1.0, 1.0, 1.0),
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		disk_tex,
		GET_VARIABLE_NAME(cone_pres));
	...
	light_sphere_0.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(light_sphere_0));
	...
	light_sphere_1.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(light_sphere_1));
	...
	light_sphere_2.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(light_sphere_2));
	...
	light_sphere_3.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		Vector4(1.0f, 1.0f, 1.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(light_sphere_3));
	...
	light_sphere_4.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		NULL,
		GET_VARIABLE_NAME(light_sphere_4));
	...
	light_sphere_5.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		Vector4(0.0f, 1.0f, 0.0f, 1.0f),
		NULL,
		GET_VARIABLE_NAME(light_sphere_5));
	...
	light_sphere_6.buildSphere(GL_TRIANGLES, 0.5, 15.0, 15.0,	// radius, latitude, longitude
		...
		NULL,
		GET_VARIABLE_NAME(light_sphere_6));
}

void GameScene::renderShapes() {
...
void GameScene::renderSolarSystem() {
	// SOLAR SYSTEM START //
...
void GameScene::renderRightWall() {
	glPushMatrix(); {
...
void GameScene::updateVariables() {
	angle += 0.7f;
	/*floor.rotate(angle);
...
void GameScene::update(float dt) {
	...
	if (input->isKeyDown('3') || 
		input->isMiddleMouseButtonPressed()) {
			camera = &topDownCamera;
			input->SetKeyUp('3');
	}
	// Press B to toggle blending mode
	if (input->isKeyDown('b') || 
		input->isKeyDown('B') ) { // is B pressed and bp FALSE?
	...
	if (input->isKeyDown('x') || 
		input->isKeyDown('X') ||
		input->isScrollUpMouseWheel() ) {
			sphere_1._scale.operator+=(Vector3(0.1f, 0.1f, 0.1f));
			input->setScrollUpMouseWheel(false);
	}
	// Put Y scale up
	if (input->isKeyDown('y') || 
		input->isKeyDown('Y') ||
		input->isScrollDownMouseWheel() ) {
			sphere_1._scale.operator-=(Vector3(0.1f, 0.1f, 0.1f));
...
void GameScene::renderTextOutput() {
	...
	//sprintf_s(sphere_1_text, "sphere_1 time taken: %f", sphere_1._time_taken);
	//sprintf_s(sphere_1_text, "sphere_2 time taken: %f", sphere_1._time_taken);
	//std::string message1 = "Hello" + std::to_string(sphere_1._time_taken);
	//std::string cameraPos = "Cam Pos: X:" + std::to_string(camera->getPositionX()) + " Y:" + std::to_string(camera->getPositionY()) + " Z:" + std::to_string(camera->getPositionZ());
	//displayText(-1.f, 0.96f, 0.f, 0.f, 1.f, cameraPos.c_str());
	////displayText(-1.f, 0.96f, 0.f, 0.f, 1.f, mouseText);
	//displayText(-1.f, 0.90f, 1.f, 0.f, 0.f, cameraPos.c_str());
	...
	//displayText(-1.f, 0.48f, 1.f, 0.f, 0.f, sphere_1_text);
	
	displayText(-1.f, 0.96f, 0.f, 0.f, 1.f, fps);

	float initialyPos = 0.90f;
	/*char* var_name = GET_VARIABLE_NAME(sphere_1);
	
	std::map<std::string, __int64> map;
	map.emplace(GET_VARIABLE_NAME(sphere_1), 12);
	for (auto& item : map)
	{
		std::string message = item.first + " time taken " + std::to_string(item.second);
		displayText(-1.f, initialyPos, 1.f, 0.f, 0.f, message.c_str());
	}*/
...
void GameScene::displayText(float x, float y, float r, float g, float b, const char* string) {
	// Get Lenth of string
	int j = strlen(string);
...
void GameScene::setLightAmbient(float x, float y, float z, float w, GLfloat* lightAmbient) {
	lightAmbient[0] = x;
	lightAmbient[1] = y;
	lightAmbient[2] = z;
	lightAmbient[3] = w;
...
void GameScene::setLightDiffuse(float x, float y, float z, float w, GLfloat* lightDiffuse) {
	lightDiffuse[0] = x;
	lightDiffuse[1] = y;
	lightDiffuse[2] = z;
	lightDiffuse[3] = w;
...
void GameScene::setLightPosition(float x, float y, float z, float w, GLfloat* lightPosition) {
	lightPosition[0] = x;
	lightPosition[1] = y;
	lightPosition[2] = z;
	lightPosition[3] = w;
...
void GameScene::setLightSpecular(float x, float y, float z, float w, GLfloat* lightSpecular) {
	lightSpecular[0] = x;
	lightSpecular[1] = y;
	lightSpecular[2] = z;
	lightSpecular[3] = w;
...
void GameScene::setSpotDirection(float x, float y, float z, GLfloat* spotDirection) {
	spotDirection[0] = x;
	spotDirection[1] = y;
	spotDirection[2] = z;
}
// Setting light with Vector4 and Vector3
void GameScene::setLightAmbient(Vector4 ambient, GLfloat* lightAmbient) {
...
void GameScene::setLightDiffuse(Vector4 diffuse, GLfloat* lightDiffuse) {
...
void GameScene::setLightPosition(Vector4 position, GLfloat* lightPosition) {
...
void GameScene::setLightSpecular(Vector4 specular, GLfloat* lightSpecular) {
...
void GameScene::setSpotDirection(Vector3 spot_direction, GLfloat* spotDirection) {
	spotDirection[0] = spot_direction.x;
	spotDirection[1] = spot_direction.y;
	spotDirection[2] = spot_direction.z;
}

void GameScene::setLightShininess(GLfloat *arg, GLfloat* lightShininess) {
...
void GameScene::setLightCutOff(GLfloat* arg, GLfloat& lightCutOff) {
	lightCutOff = arg[0];
}
...
void GameScene::generateShadowMatrix(float light_pos[4], GLfloat floor[12]) {
